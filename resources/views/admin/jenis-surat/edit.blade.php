...
@section('page_title', 'Ubah Jenis Surat')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="card p-8">
        <form action="{{ route('jenis-surat.update', $item->id) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <x-text-input id="nama" name="nama" type="text" class="block w-full" :value="old('nama', $item->nama)" required />
                        <x-input-error class="mt-2" :messages="$errors->get('nama')" />
                    </div>

                    <div class="space-y-2">
                        <x-input-label for="kode" :value="__('Kode Singkat (Unique)')" />
                        <x-text-input id="kode" name="kode" type="text" class="block w-full" :value="old('kode', $item->kode)" required />
                        <x-input-error class="mt-2" :messages="$errors->get('kode')" />
                    </div>

                    <div class="space-y-2">
                        <x-input-label for="urutan" :value="__('Urutan Tampil')" />
                        <x-text-input id="urutan" name="urutan" type="number" class="block w-full" :value="old('urutan', $item->urutan)" />
                ...
                <div class="space-y-2">
                    ...
                    <textarea id="keterangan" name="keterangan" rows="3" class="block w-full border-slate-200 rounded-lg text-sm focus:border-brand-blue-500 focus:ring-brand-blue-500">{{ old('keterangan', $item->keterangan) }}</textarea>
                    <x-input-error class="mt-2" :messages="$errors->get('keterangan')" />
                </div>

                <div class="space-y-2">
                    <x-input-label for="persyaratan" :value="__('Persyaratan (List)')" />
                    <textarea id="persyaratan" name="persyaratan" rows="4" class="block w-full border-slate-200 rounded-lg text-sm focus:border-brand-blue-500 focus:ring-brand-blue-500" placeholder="Satu baris per persyaratan">{{ old('persyaratan', $item->persyaratan) }}</textarea>
